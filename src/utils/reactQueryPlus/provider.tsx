...
export const ReactQueryPlusSetter = React.createContext<
  React.Dispatch<{ type?: string; value: any; hash: string }>
>(() => {});
...
export const ReactQueryPlusProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [state, setState] = useReducer<
    Reducer<MyState, { type?: string; value: any; hash: string }>
  >(
    (state, action) => {
      switch (action.type) {
        case "loading":
